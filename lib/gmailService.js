...
export async function scanEmailsForMoney(accessToken, userId) {
  try {
    // First, get count of ALL money-related emails (for transparency)
    const allMessages = await searchMoneyEmails(accessToken, {
      maxResults: 50 // Get up to 50 to show total available
    });

    const totalEmailsFound = allMessages.length;
    console.log(`📧 Found ${totalEmailsFound} money-related emails in Gmail`);
    ...
    // Only analyze first 20 to fit in timeout
    ...
    console.log(`📧 Analyzing ${emailContents.length} emails with AI...`);
    ...
    for (const email of emailContents) {
      try {
        analyzedCount++;
        console.log(`📝 Analyzing email ${analyzedCount}/${emailContents.length}: ${email.subject.substring(0, 50)}...`);
    ...
    console.log(`✅ Scan complete: ${emailContents.length} emails analyzed, ${opportunities.length} opportunities found`);

    // Include email subjects for verification/debugging
    const emailSubjects = emailContents.map(email => ({
      subject: email.subject,
      from: email.from,
      date: email.date
    }));
    ...
    return {
      ...
      message: `Found ${opportunities.length} potential money opportunities in ${emailContents.length} of ${totalEmailsFound} emails`
    };
  } catch (error) {
